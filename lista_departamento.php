...
$current_ui_url = "lista_departamento.php?" . http_build_query(['p' => $p_actual, 'q' => $busqueda]);

function renderDePagination($p_actual, $total_paginas, $busqueda) {
...
if (isset($_GET['ajax'])) {
    ...
    echo renderDePagination($p_actual, $total_paginas, $busqueda);
    exit;
}
...
<div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
    <div>
        <h2 class="text-2xl font-bold text-slate-800 tracking-tight">Lista de Departamentos</h2>
        <p class="text-slate-500 text-sm font-medium">Organización y ubicación de la empresa</p>
    </div>
    <div class="flex flex-wrap gap-2 w-full md:w-auto">
        <div class="relative flex-1 md:flex-none">
            <input type="text" id="busqueda" value="<?php echo htmlspecialchars($busqueda); ?>" placeholder="Buscar depto..." class="pl-10 pr-4 py-2.5 border border-slate-200 bg-white rounded-2xl focus:outline-none focus:ring-2 focus:ring-blue-500 w-full md:w-64 transition-all shadow-sm">
        ...
        <a href="crear_departamento.php" class="whitespace-nowrap bg-blue-600 text-white px-6 py-2.5 rounded-2xl uppercase font-black hover:bg-blue-700 transition-all shadow-lg shadow-blue-200 hover:scale-105">Crear Depto</a>
    </div>
</div>
...
<div id="contenedor-tabla-completo">
    <div class="bg-white rounded-[2rem] shadow-sm border border-slate-200 overflow-hidden">
        <table class="w-full border-collapse">
            ...
            <tbody id="tabla-departamentos" class="divide-y divide-slate-100">
                <?php include "lista_departamento_rows.php"; ?>
            </tbody>
        </table>

        <div id="contenedor-paginacion">
            <?php echo renderDePagination($p_actual, $total_paginas, $busqueda); ?>
        </div>
</div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const tableBody = document.getElementById('tabla-departamentos');
    ...
    function actualizarTabla(pagina = 1, push = true) {
        ...
        const url = `lista_departamento.php?ajax=1&p=${pagina}&q=${encodeURIComponent(busqueda)}`;
